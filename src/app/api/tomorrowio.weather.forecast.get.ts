...
import { ForecastWeatherData } from "./tomorrowio.weather.forecast.data.model";


/**
 * Retrieves forecast weather data from the Tomorrow.io API.
 * 
 * @param lat - The latitude of the location.
 * @param lng - The longitude of the location.
 * @returns A promise that resolves to the forecast weather data.
 * @throws An error if there is an HTTP error or if the API response has an invalid data structure.
 */
export async function getTomorrowioForecastData<T extends ForecastWeatherData>( lat:number, lng :number): Promise<T> {
    const options = {
        method: 'GET',
        headers: {
